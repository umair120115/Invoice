...
import { Bell, Search, Users, CheckSquare, Square, Send, Filter, X, ChevronLeft, ChevronRight, Loader, MessageSquare } from 'lucide-react';
import api from '../api';

const MobileNotificationApp = () => {
...
//   const [apiUrl] = useState('http://127.0.0.1:8000/');4
  ...
  // Fetch users with incomplete orders
  const fetchUsers = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await api.get(`/api/admincontrol/user-order-summary/?${params.toString()}`
        // headers: {
    ...
    setLoading(false);
  }, [apiUrl, selectAllPages, allSelectedUsers]);
  ...
  const handleSendNotification = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${apiUrl}/api/admincontrol/send-notification/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
        ...
        <div style={styles.leftPanel}>
          <div style={styles.filterSection}>
            <div style={styles.filterHeader}>
              <Filter size={20} />
              <h2 style={styles.sectionTitle}>Users with Incomplete Orders</h2>
            </div>
            
            <div style={styles.filterInfo}>
              <div style={styles.infoBox}>
                <MessageSquare size={16} />
                <span>Users with items in cart but order not placed</span>
              </div>
            ...
            <div style={styles.usersList}>
              {loading ? (
                ...
                <div style={styles.emptyState}>
                  ...
                  <p>No users found with incomplete orders</p>
                  <p style={styles.emptyHint}>All users have completed their orders!</p>
                </div>
              ) : (
                users.map((user) => (
                  <div 
...
export default MobileNotificationApp;